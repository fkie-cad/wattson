...
class StateEstimator(Thread):
    ...
    def __init__(self,
        ...
        self.net_lock: Lock = threading.Lock()
    ...
    def estimate_state(self) -> None:
        ...
        self.power_grid_model.clean_measurements()

        with self.net_lock:
            # Copy the network before estimation
            pnet = deepcopy(self.power_net)
        ...
        with self._measurement_lock:
            ...
            for key, measurement in self._measurements.items():
                if measurement.is_valid(self.decay, self.virtual_time):
                    self.power_grid_model.add_estimation_measurement(measurement.grid_value, measurement.value)
                    post += 1
                    ...
                    if measurement.get_element_type() == "line":
                        ...
                        if abs(measurement.value) < threshold * max_ka:
                            self.logger.info(
                                f"Line {index} detected as faulty ({measurement.value} of {max_ka})")
                            faulty_lines.add(index)
                            # pnet.line.at[index, "in_service"] = False
                            # Open a switch for the wrapper
                            any_opened = False
