...
class OfficeNAT(ScenarioExtension):
    def extend_pre_physical(self):
        ...
        try:
            ...
            self.co_simulation_controller.logger.error(f"[OfficeNAT] Could not find NAT switch: {e=}")
            return

        nat_ip = network_emulator.get_unused_ip(nat_switch.get_subnets(include_management=False)[0])

        # Add NAT
        self.co_simulation_controller.logger.info(f"[OfficeNAT] Adding NAT to {nat_switch_name} with IP {nat_ip}")
        nat = WattsonNetworkNAT(id="office-nat")
