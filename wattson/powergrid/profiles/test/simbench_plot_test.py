...
loads = set()
static_generators = set()
for load_id in power_net.load.index:
    e = ("load", int(load_id))
    loads.add(e)
for static_generator_id in power_net.static_generator.index:
    e = ("sgen", int(static_generator_id))
    static_generators.add(e)
loads = frozenset(loads)
static_generators = frozenset(static_generators)
element_collection.add(("Load (MW)", loads))
element_collection.add(("Generation (MW)", static_generators))
